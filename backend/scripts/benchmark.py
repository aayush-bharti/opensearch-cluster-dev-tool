...
class OpenSearchBenchmarker:
    ...
    def __init__(self, config: Dict[str, Any], workflow_timestamp: str = None):
        ...
        # Validate AWS credentials and get region (same pattern as build/deploy scripts)
    ...
    def run_benchmark(self):
        ...
        try:
            ...
            if not self.config.get("workload_type"):
                raise ValueError("workload_type is required in config")
            if not self.config.get("cluster_endpoint"):
                raise ValueError("cluster_endpoint is required in config")
            
            # Ensure values are strings
            workload_type = str(self.config["workload_type"]).strip()
            cluster_endpoint = str(self.config["cluster_endpoint"]).strip()
            ...
            pipeline = self.config.get("pipeline", "benchmark-only")
            ...
            # Create benchmark output (needed for benchmark_result)
            ...
            if results_s3_info:
                ...
                benchmark_result["message"] = "Benchmark completed successfully with S3 upload"
                logger.info("✅ Benchmark completed successfully with S3 upload")
            else:
                benchmark_result["message"] = "Benchmark completed successfully (S3 upload failed)"
    ...
    def upload_benchmark_results_to_s3(self, benchmark_result: Dict[str, Any], timing_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create benchmark results metadata (following build pattern)
            benchmark_metadata = {
                "benchmark_status": benchmark_result.get("status"),
                "benchmark_message": benchmark_result.get("message"),
                "benchmark_id": benchmark_result.get("benchmark_id"),
                "results_location": benchmark_result.get("results_location"),
                "config": self.config
            }
            
            # Use the new upload method with timing data
            result = uploader.upload_benchmark_results(benchmark_metadata, timing_data)
    ...
    def format_benchmark_text_results(self, raw_content: str) -> Dict[str, Any]:
        ...
        formatted_results = {
            "benchmark_metadata": {
                "benchmark_id": self.benchmark_id,
                "workload_type": self.config.get("workload_type"),
