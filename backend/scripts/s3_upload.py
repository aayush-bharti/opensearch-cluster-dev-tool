...
class S3Uploader:
    ...
    def upload_build_manifest(self, manifest_path: str) -> Dict[str, Any]:
        ...
        s3_key = f"{self.workflow_timestamp}/build/{filename}"
    ...
    def upload_build_results(self, results_data: Dict[str, Any], timing_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """Upload build results to S3 using new structure: timestamp/build/build-results.json"""
        filename = f"build-results.json"
        s3_key = f"{self.workflow_timestamp}/build/{filename}"
    ...
    def upload_deploy_results(self, results_data: Dict[str, Any], timing_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """Upload deploy results to S3 using new structure: timestamp/deploy/deploy-results.json"""
        filename = f"deploy-results.json"
        s3_key = f"{self.workflow_timestamp}/deploy/{filename}"
    ...
    def upload_benchmark_results(self, results_data: Dict[str, Any], timing_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """Upload benchmark results to S3 using new structure: timestamp/benchmark/benchmark-results.json"""
        filename = f"benchmark-results.json"
        s3_key = f"{self.workflow_timestamp}/benchmark/{filename}"
    ...
    # upload the task results to S3
    def upload_task_results(self, task_type: str, results_data: Dict[str, Any], prefix: str = None) -> Dict[str, Any]:
        """Upload task results (build, deploy, benchmark) to S3."""
        if task_type == "build":
            return self.upload_build_results(results_data)
        elif task_type == "deploy":
            return self.upload_deploy_results(results_data)
        elif task_type == "benchmark":
            return self.upload_benchmark_results(results_data)
        else:
            raise ValueError(f"Unknown task type: {task_type}")
    
    # verify the access to the uploaded file
    def verify_access(self, s3_key: str) -> bool:
        """Verify that an uploaded file can be accessed."""
        try:
            response = self.s3_client.head_object(Bucket=self.bucket_name, Key=s3_key)
            logger.info(f"✅ File verified: {response['ContentLength']} bytes")
            return True
        except ClientError as e:
